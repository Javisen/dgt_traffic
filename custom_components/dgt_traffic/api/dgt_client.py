...
class DGTClient:
    ...
    async def get_incidents(self, max_age_days: int = 7) -> List[Dict]:
        ...
        try:
            _LOGGER.debug("Fetching incidents from DGT API")
            ...
            async with self._session.get(
                ...
                _LOGGER.debug("Received %s bytes from DGT", len(xml_content))
...
class DGTXMLParser:
    ...
    async def parse_xml(self, xml_content: str, max_age_days: int = 7) -> List[Dict]:
        ...
        try:
            ...
            _LOGGER.debug("Found %s situations in XML", len(situations))
            ...
            _LOGGER.debug("Parsed %s recent incidents", len(incidents))
            ...
            _LOGGER.exception("Error in XML parsing: %s", err)
    ...
    def _parse_situation(self, situation_elem) -> Optional[Dict]:
        ...
        try:
            ...
            _LOGGER.debug("Error parsing situation: %s", err)
            return None
    ...
    def _parse_situation_record(self, record_elem) -> Optional[Dict]:
        ...
        try:
            ...
            # Record type (extract from xsi:type)
            ...
            _LOGGER.debug("Error parsing record: %s", err)
            return None
    ...
    def _extract_location(self, record_elem) -> Dict:
        ...
        try:
            ...
            _LOGGER.debug("Error extracting location: %s", err)
    ...
    def _generate_description(self, record_data: Dict) -> str:
        ...
        try:
            ...
            _LOGGER.debug("Error generating description: %s", err)
    ...
    def _parse_datetime(self, dt_str: str) -> Optional[str]:
        ...
        try:
            ...
            return dt_str  # Return original if can't parse
    ...
    async def _parse_with_regex(
        ...
        incidents = []
        _LOGGER.warning("Using regex fallback parsing")
        ...
        try:
            ...
            _LOGGER.debug("Regex found %s situations", len(situations))

            # Simplified parsing for now
            # We'll implement full regex parsing if needed
